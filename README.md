...
The implementation has been validated against an independent code (the
[eT program](https://etprogram.org)) to better than 1 nHa.
...
- **Dipole self-energy (DSE)** is folded directly into the one- and
  two-electron integrals before diagonalization,

  ```
  h_pq     -> h_pq     + 0.5 * (D·D)_pq
  (pq|rs)  -> (pq|rs)  + D_pq * D_rs
  ```

  with the *asymmetric* 0.5 / 1.0 split required by how PySCF's
  `absorb_h1e` folds one-electron terms into the two-electron tensor — a
  naive symmetric 0.5 / 0.5 split silently gives the wrong energy.
...
- **Finite-field response**: μᵢ = −∂E/∂Fᵢ and αᵢⱼ = −∂²E/∂Fᵢ∂Fⱼ, evaluated
  with a Richardson-extrapolated central-difference stencil (two step
  sizes, h and 2h) to push the leading truncation error from order h² to
  order h⁴. The number of field points needed depends on how the cavity
  mode is oriented relative to the molecular axis (7–9 points along the
  axis, up to 25 for a fully general off-axis mode).
...
| `--lam` | `0.0` | Coupling strength λ (a.u.) |
| `--omega` | `0.4687` | Cavity mode frequency ω_c (a.u.) |
...
| `--nboson` | `6` | Photon Fock-space cutoff N_p |
...
| `--verify` | off | At λ = 0, cross-check against plain PySCF FCI |
